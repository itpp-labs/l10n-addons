...
class res_partner(osv.osv):
    ...
    _columns = {
        ...
        "address_formatted": fields.function(
            _format_address, string="Formatted Address", type="char", store=False
        ),
...
class Bank(osv.osv):
    def name_search(
        self, cr, uid, name="", args=[], operator="ilike", context=None, limit=80
    ):
        ids = self.search(
            cr,
            uid,
            ["|", ("bic", operator, name), ("name", operator, name)] + args,
            limit=limit,
            context=context,
        )
    ...
    def update_banks(self, cr, uid, ids, context=None):
        ...
        try:
            ...
            raise osv.except_osv(
                _("Bad URL for bnkseek.txt!"), _("Fix the URL and try again.")
            )
        ...
        country_ids = self.pool.get("res.country").search(
            cr, uid, [("name", "=", our_country)], context=context
        )
        ...
        try:
            ...
            raise osv.except_osv(
                _("Bad URL for bnkdel.txt!"), _("Fix the URL and try again.")
            )
