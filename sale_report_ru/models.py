...
class res_partner(osv.Model):
    ...
    _columns = {
        "represented_by": fields.char(
            "Represented by", size=256, help="String for contracts"
        ),
        "default_bank_id": fields.function(
            _get_default_bank_id, type="many2one", obj="res.partner.bank"
        ),
    }
...
def money_to_words(amount, code):
    ...
    copek = (
        numeral.choose_plural(int(copek_num), CURRENCY_CENTS_RU[code])
        if code in CURRENCY_CENTS_RU
        else ""
    )
    if copek:
...
class sale_order(osv.Model):
    ...
    _columns = {
        "date_acceptance": fields.date(
            "Acceptance act date", required=False, readonly=False
        ),
        "amount_total_in_words": fields.function(
            _get_amount_in_words, string="Amount in words", type="char"
        ),
    }
