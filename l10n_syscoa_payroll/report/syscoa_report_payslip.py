...
class ErgobitPayslipReport(report_sxw.rml_parse):
    ...
    def get_payslip_lines_main(self, lines_ids):
        ...
        category_ids = env["hr.salary.rule.category"].search(
            [
                (
                    "code",
                    "not in",
                    [self.C_COMP, self.C_COMPP, self.C_NET, self.C_TOTA, self.C_TOTM],
                )
            ]
        )
        ...
        for idx in range(len(lines_ids)):
            if (lines_ids[idx].appears_on_payslip is True) and (
                lines_ids[idx].category_id in category_ids
            ):
    ...
    def get_quantity(self, psline):
        if (
            round(float(psline.quantity), 2) == 1.00
            or round(float(psline.quantity), 2) == 0.00
        ):
            return ""
    ...
    def get_rate_salarial(self, psline):
        if (
            round(float(psline.rate), 2) == 100.00
            or round(float(psline.rate), 2) == 0.00
        ):
            return ""
    ...
    def get_gain_salarial(self, psline):
        ...
        if psline.code == "100":
            if (
                psline.slip_id.contract_id.time_mod == "fixed"
                and psline.slip_id.contract_id.time_fixed
                == psline.slip_id.get_worked_hours("WORKED")
                and psline.slip_id.get_worked_hours("WORK100")
                == psline.slip_id.get_worked_hours("WORKED")
            ):
    ...
    def get_deduction_salarial(self, psline):
        env = odoo.api.Environment(self.cr, self.uid, {})
        if round(float(psline.total), 2) == 0.00:
            tax_cat_ids = env["hr.salary.rule.category"].search(
                [("code", "in", [self.C_TAX])]
            )
    ...
    def get_rate_patronal(self, psline):
        ...
        payslip_line_comp_id = pooler.search(
            [("slip_id", "=", psline.slip_id.id), ("code", "=", psline_code_comp)]
        )
    ...
    def get_contribution_patronal_plus(self, psline):
        ...
        pooler = env["hr.payslip.line"]
        payslip_line_compp_id = pooler.search(
            [("slip_id", "=", psline.slip_id.id), ("code", "=", psline_code_compp)]
        )
    ...
    def get_payslip_lines_total(self, lines_ids, timeref="MONTH"):
        ...
        if timeref == "MONTH":
            ...
            category_ids = env["hr.salary.rule.category"].search(
                [("code", "in", [self.C_TOTM, self.C_NET])]
            )
        else:
            res = dict.fromkeys(
                [
            ...
            if payslip_line_total:
                for linx in range(len(payslip_line_total)):
                    if payslip_line_total[linx].code in res.keys():
                        d = dict.fromkeys(
                            [payslip_line_total[linx].code],
                            payslip_line_total[linx].total,
                        )
                        res.update(d)
