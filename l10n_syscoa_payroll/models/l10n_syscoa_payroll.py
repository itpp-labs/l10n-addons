...
class ResCompany(models.Model):
    ...
    css_percentage = fields.Selection(
        (("one", "1,0%"), ("three", "3,0%"), ("five", "5,0%")),
        required=True,
        default="three",
        string="CSS - Accident de travail",
    )
...
class HrContract(models.Model):
    ...
    def _get_mutual_insurance_empl(self):
        for line in self:
            if line.mutual_insurance_empl_manual:
                line.mutual_insurance_employee = float(
                    line.mutual_insurance_empl_manual_input
                )
            else:
    ...
    def _get_mutual_insurance_comp(self):
        for line in self:
            if line.mutual_insurance_comp_manual:
                line.mutual_insurance_comp = float(
                    line.mutual_insurance_comp_manual_input
                )
            else:
    ...
    governmental_org = fields.Boolean(
        compute="_read_governmental_org", method=True, string="Organisme gouvernemental"
    )
    ...
    time_mod = fields.Selection(
        (
            ("fixed", "Heures de travail fixes"),
            ("variable", "Heures de travail variables"),
        ),
        string="Mode de gestion du temps",
        required=True,
        default="fixed",
    )
    time_fixed = fields.Float(
        string="Nombre d'heures fixe prévues", default=173.33, required=True
    )

    pay_mod = fields.Selection(
        (("Virement", "Virement"), ("Cheque", "Chèque"), ("Espece", "Espèce")),
        "Mode de paiement préféré",
    )
    qualif = fields.Char("Qualification")
    niveau = fields.Char(compute="_read_social_parts", method=True, string="Niveau")
    coef = fields.Integer(
        compute="_read_coefficient", method=True, string="Coefficient"
    )
    ...
    gross_invoice = fields.Float(
        compute="_get_gross_invoice",
        method=True,
        string="Montant brut facturé",
        digits_compute=dp.get_precision("Account"),
    )
    additional_salary = fields.Float(
        "Sursalaire", digits_compute=dp.get_precision("Payroll")
    )

    union_fee = fields.Float(
        "Cotisation syndicale", digits_compute=dp.get_precision("Payroll")
    )

    performance_bonus = fields.Float(
        "Prime de rendement", digits_compute=dp.get_precision("Payroll")
    )
    gratification = fields.Float(
        "Gratification", digits_compute=dp.get_precision("Payroll")
    )

    seniority_date_manual = fields.Boolean(
        "Date de début",
        help="La date de début pour le calcul de l'ancienneté est normalement eǵale à la date de début du contrat. Mais vous pouvez la saisir manuellement en activant ce bouton.",
    )
    seniority_date_manual_input = fields.Date(
        "Date de début",
        digits_compute=dp.get_precision("Payroll"),
        help="La date de début pour le calcul de l'ancienneté est normalement eǵale à la date de début du contrat. Mais vous pouvez la saisir manuellement en activant le bouton à côté.",
    )
    seniority_date = fields.Date(
        compute="_get_seniority_date",
        method=True,
    ...
    seniority_allowance_manual = fields.Boolean(
        "Indemnité d'ancienneté",
        help="L'indemnité d'ancienneté est normalement calculée automatiquement. Mais vous pouvez le saisir manuellement en activant ce bouton.",
    )
    seniority_allowance_manual_input = fields.Float(
        "Indemnité d'ancienneté",
        digits_compute=dp.get_precision("Payroll"),
        help="L'indemnité d'ancienneté est normalement calculée automatiquement. Mais vous pouvez le saisir manuellement en activant le bouton à côté.",
    )
    seniority_allowance = fields.Float(
        compute="_calculate_seniority_allowance",
        method=True,
        string="Indemnité d'ancienneté",
        digits_compute=dp.get_precision("Account"),
        help="L'indemnité d'ancienneté est normalement calculée automatiquement. Mais vous pouvez le saisir manuellement en activant le bouton à côté.",
    )

    yearly_max_leaves = fields.Float("Droit de congé par année (en jours)")
    attribute_leave_days = fields.Boolean(
        "Attribuer les congés automatiquement", default=True
    )
    leave_days_manual = fields.Boolean(
        "Nombre de jours à attribuer par mois",
        help="Le nombre de jours est normalement eǵale à 2. Mais vous pouvez saisir un autre nombre en cochant le bouton à côté.",
    )
    leave_days_manual_input = fields.Float(
        "Nombre de jours à attribuer par mois",
        default=2.5,
        help="Le nombre de jours est normalement eǵale à 2. Mais vous pouvez saisir un autre nombre en cochant le bouton à côté.",
    )
    leave_days = fields.Float(
        compute="_get_leave_days",
        method=True,
        string="Nombre de jours à attribuer par mois",
        help="Le nombre de jours est normalement eǵale à 2,5. Mais vous pouvez saisir un autre nombre en cochant le bouton à côté.",
    )

    risk_bonus = fields.Float(
        "Prime de risque", digits_compute=dp.get_precision("Payroll")
    )
    home_bonus = fields.Float(
        "Prime de logement", digits_compute=dp.get_precision("Payroll")
    )
    cashpoint_bonus = fields.Float(
        "Prime de caisse", digits_compute=dp.get_precision("Payroll")
    )
    expatriation_bonus = fields.Float(
        "Prime d'expratriation", digits_compute=dp.get_precision("Payroll")
    )
    basket_bonus = fields.Float(
        "Prime de panier", digits_compute=dp.get_precision("Payroll")
    )
    responsability_bonus = fields.Float(
        "Prime de responsabilité", digits_compute=dp.get_precision("Payroll")
    )
    subjection_allowance = fields.Float(
        "Indemnité de sujétion", digits_compute=dp.get_precision("Payroll")
    )

    food_advantage = fields.Float(
        "Avantage pour nourriture", digits_compute=dp.get_precision("Payroll")
    )
    domesticity_bonus = fields.Float(
        "Avantage pour logement et domesticité",
        digits_compute=dp.get_precision("Payroll"),
    )
    family_advantage = fields.Float(
        "Avantages familiaux", digits_compute=dp.get_precision("Payroll")
    )
    company_car_advantage = fields.Float(
        "Avantage pour véhicule de fonction", digits_compute=dp.get_precision("Payroll")
    )
    company_phone_advantage = fields.Float(
        "Avantage pour téléphone", digits_compute=dp.get_precision("Payroll")
    )
    water_electricity_advantage = fields.Float(
        "Avantage pour fourniture d'eau et d'électricité",
        digits_compute=dp.get_precision("Payroll"),
    )

    kilometer_refund = fields.Float(
        "Indemnité kilométrique", digits_compute=dp.get_precision("Payroll")
    )
    transport_refund = fields.Float(
        "Indemnité de transport", digits_compute=dp.get_precision("Payroll")
    )
    transport_refund_frequence = fields.Selection(
        (("day", "Jour"), ("month", "Mois")),
        "Frequence des indemnités de transport",
        required=True,
        default="month",
    )
    meal_voucher = fields.Float(
        "Bons de repas", digits_compute=dp.get_precision("Payroll")
    )
    meal_voucher_frequence = fields.Selection(
        (("day", "Jour"), ("month", "Mois")),
        "Frequence des bons de repas",
        required=True,
        default="month",
    )

    mutual_insurance_empl_manual = fields.Boolean(
        "Cotisation salariale",
        help="La cotisation à la mutuelle d'assurance est normalement calculée automatiquement. Mais vous pouvez la saisir manuellement en activant ce bouton.",
    )
    mutual_insurance_empl_manual_input = fields.Float(
        "Cotisation salariale",
        digits_compute=dp.get_precision("Payroll"),
        help="La cotisation à la mutuelle d'assurance est normalement calculée automatiquement. Mais vous pouvez la saisir manuellement en activant le bouton à côté.",
    )
    mutual_insurance_empl = fields.Float(
        compute="_get_mutual_insurance_empl",
        method=True,
        string="Cotisation salariale",
        digits_compute=dp.get_precision("Account"),
        help="La cotisation à la mutuelle d'assurance est normalement calculée automatiquement. Mais vous pouvez la saisir manuellement en activant le bouton à côté.",
    )

    mutual_insurance_comp_manual = fields.Boolean(
        "Cotisation patronale",
        help="La cotisation à la mutuelle d'assurance est normalement calculée automatiquement. Mais vous pouvez la saisir manuellement en activant ce bouton.",
    )
    mutual_insurance_comp_manual_input = fields.Float(
        "Cotisation patronale",
        digits_compute=dp.get_precision("Payroll"),
        help="La cotisation à la mutuelle d'assurance est normalement calculée automatiquement. Mais vous pouvez la saisir manuellement en activant le bouton à côté.",
    )
    mutual_insurance_comp = fields.Float(
        compute="_get_mutual_insurance_comp",
        method=True,
        string="Cotisation patronale",
        digits_compute=dp.get_precision("Account"),
        help="La cotisation à la mutuelle d'assurance est normalement calculée automatiquement. Mais vous pouvez la saisir manuellement en activant le bouton à côté.",
    )

    dirtiness_allowance = fields.Float(
        "Prime de salissure", digits_compute=dp.get_precision("Payroll")
    )
...
class HrPayslip(models.Model):
    ...
    def action_payslip_done(self):
        for payslip in self:
            if not payslip.employee_id.address_home_id:
                raise UserError(
                    _(
                        "L'employé '%s' n'a pas d'adresse personnelle. \nVeuillez renseigner son adresse personnelle dans la fiche de l'employé, \nonglet 'Information personnelle'"
                    )
            ...
            if payslip.leave_days_won > 0:
                if payslip.state == "verify":
                    if payslip.credit_note:
                        ...
                        payslip.employee_id.remaining_leaves = (
                            payslip.remaining_leaves + payslip.leave_days_won
                        )
        ...
        for line in self:
            if line.is_waste_collector:
                ...
                date_1 = datetime.strptime(
                    str(str(date_1.year) + "-" + str(date_1.month) + "-" + str(26)),
                    "%Y-%m-%d",
                ) + relativedelta.relativedelta(months=-1)
                date_1 = date_1.strftime("%Y-%m-%d")
                date_2 = datetime.strptime(str(line.date_to), "%Y-%m-%d")
                date_2 = datetime.strptime(
                    str(str(date_2.year) + "-" + str(date_2.month) + "-" + str(25)),
                    "%Y-%m-%d",
                )
                ...
                if analytic_account == []:
                    ...
                    cr = self.env.cr
                    cr.execute(
                        """SELECT COALESCE(SUM(amount), 0.0) as amount, COALESCE(SUM(unit_amount), 0.0) as unit_amount \
                    ...
                    line.update(
                        {
                            "quantity_delivred": result["unit_amount"],
                            "amount_invoiced": result["amount"],
                        }
                    )
        ...
        for line in self:
            ...
            if line.type in ["leaves", "mix"]:
                if line.holiday_allowance_manual:
                    ...
                    if line.contract_id:
                        gross_of_current_month = (
                            line.contract_id.wage + line.contract_id.additional_salary
                        )
                    ...
                    line.holiday_allowance = float(
                        gross_of_current_month + gross_of_previous_month
                    )
    ...
    def _compute_leave_days(self):
        ...
        for record in self:
            ...
            if employee_id:
                ...
                hr_holidays_status = hr_holidays_status_pooler.search(
                    [
                        (
                            "id",
                            "in",
                            [
                                employee_id.company_id.legal_holidays_status_id.id,
                ...
                leave_days = hr_holidays_status_pooler.get_days(
                    hr_holidays_status, employee_id.id
                )
                record.update(
                    {
                        ...
                        if employee_id.company_id.legal_holidays_status_id
                        else 0.0,
                        ...
                        "max_leaves": leave_days[
                            employee_id.company_id.legal_holidays_status_id.id
                        ]["max_leaves"]
                        if employee_id.company_id.legal_holidays_status_id
                        else 0.0,
                        "leaves_taken_n1": leave_days[
                            employee_id.company_id.legal_holidays_status_id_n1.id
                        ]["leaves_taken"]
                        if employee_id.company_id.legal_holidays_status_id_n1
                        else 0.0,
                        "remaining_leaves_n1": leave_days[
                            employee_id.company_id.legal_holidays_status_id_n1.id
                        ]["remaining_leaves"]
                        if employee_id.company_id.legal_holidays_status_id_n1
                        else 0.0,
                        "max_leaves_n1": leave_days[
                            employee_id.company_id.legal_holidays_status_id_n1.id
                        ]["max_leaves"]
                        if employee_id.company_id.legal_holidays_status_id_n1
                        else 0.0,
                        "leaves_taken_n2": leave_days[
                            employee_id.company_id.legal_holidays_status_id_n2.id
                        ]["leaves_taken"]
                        if employee_id.company_id.legal_holidays_status_id_n2
                        else 0.0,
                        "remaining_leaves_n2": leave_days[
                            employee_id.company_id.legal_holidays_status_id_n2.id
                        ]["remaining_leaves"]
                        if employee_id.company_id.legal_holidays_status_id_n2
                        else 0.0,
                        "max_leaves_n2": leave_days[
                            employee_id.company_id.legal_holidays_status_id_n2.id
                        ]["max_leaves"]
                        if employee_id.company_id.legal_holidays_status_id_n2
                        else 0.0,
    ...
    type = fields.Selection(
        (("salary", "Salaire"), ("leaves", "Congé"), ("mix", "Salaire et Congé")),
        readonly=True,
    ...
    pay_date = fields.Date(
        "Date de Paiement", default=lambda *a: time.strftime("%Y-%m-%d")
    )
    pay_mod = fields.Selection(
        (("Virement", "Virement"), ("Cheque", "Chèque"), ("Espece", "Espèce")),
        "Mode de Paiement",
    )
    is_waste_collector = fields.Boolean(
        compute="_get_waste_collector", method=True, store=False
    )
    quantity_delivred = fields.Float(
        compute="_calculate_rendement",
        method=True,
    ...
    leave_days_won = fields.Float(
        string="À ajouter",
        readonly=True,
        states={"draft": [("readonly", False)]},
        help="Le nombre de jours de congés acquis par l'emploi dans le mois payé.",
    )
    max_leaves = fields.Float(
    ...
    max_leaves_n1 = fields.Float(
        compute="_compute_leave_days",
        string="Acquis N-1",
        store=True,
        help="This value is given by the sum of all holidays requests with a positive value.",
    )
    leaves_taken_n1 = fields.Float(
        compute="_compute_leave_days",
        string="Pris N-1",
        store=True,
        help="This value is given by the sum of all holidays requests with a negative value.",
    )
    remaining_leaves_n1 = fields.Float(
        compute="_compute_leave_days",
        string="Restant N-1",
        store=True,
        help="Maximum Leaves Allowed - Leaves Already Taken",
    )
    max_leaves_n2 = fields.Float(
        compute="_compute_leave_days",
        string="Acquis N-2",
        store=True,
        help="This value is given by the sum of all holidays requests with a positive value.",
    )
    leaves_taken_n2 = fields.Float(
        compute="_compute_leave_days",
        string="Pris N-2",
        store=True,
        help="This value is given by the sum of all holidays requests with a negative value.",
    )
    remaining_leaves_n2 = fields.Float(
        compute="_compute_leave_days",
        string="Restant N-2",
        store=True,
        help="Maximum Leaves Allowed - Leaves Already Taken",
    )
    holiday_allowance_manual = fields.Boolean(
        "Montant brut de l'indemnité",
        readonly=True,
        states={"draft": [("readonly", False)]},
        help="Le montant du droit de congé est normalement calculé automatiquement. Mais vous pouvez le saisir manuellement en activant le bouton à côté.",
    )
    holiday_allowance_manual_input = fields.Float(
        "Montant brut de l'indemnité",
        readonly=True,
        states={"draft": [("readonly", False)]},
        digits_compute=dp.get_precision("Payroll"),
    ...
    holiday_allowance = fields.Float(
        compute="_get_holiday_allowance",
        method=True,
        string="Montant brut de l'indemnité",
        store=True,
        digits_compute=dp.get_precision("Payroll"),
        help="Le montant du droit de congé est normalement calculé automatiquement. Mais vous pouvez le saisir manuellement en activant le bouton à côté.",
    )
    ...
    def get_worked_day_lines(self, contract_ids, date_from, date_to):
        ...
        for contract in self.env["hr.contract"].browse(
            contract_ids
        ):  # .filtered(lambda contract: contract.working_hours):
            attendances = {
            ...
            for day in range(0, nb_of_days):
                working_hours_on_day = contract.working_hours.working_hours_on_day(
...
class HrPayslipLine(models.Model):
    ...
    rate2 = fields.Float(
        "Rate2 (%)", digits_compute=dp.get_precision("Payroll Rate"), default=100.0
    )
    amount2 = fields.Float("Amount2", digits_compute=dp.get_precision("Payroll"))
    quantity2 = fields.Float(
        "Quantity2", digits_compute=dp.get_precision("Payroll"), default=1.0
    )
    total2 = fields.Float(
